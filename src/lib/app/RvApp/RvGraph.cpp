...
#define PAD "%06zd"
    ...
    void RvGraph::removeNode(IPNode* n)
    {
    ...
    void RvGraph::initializeIPTree(const VideoModules& modules)
    {
        lockAudioFill();
        finishAudioThread();
        unlockAudioFill();
    ...
    void RvGraph::connectNewSourcesToDefaultViews()
    {
        HOP_PROF_FUNC();
                              ...
                              IPCore::SourceIPNode* mediaRepSisterSrcNode)
    {
        ...
        {
            // Make sure that the media is not loaded by default when adding a
            // media representation as we only want to load it when the media
            ...
            if (!switchGroup)
            {
                ...
                {
                    // Fast add source mechanism which postpones the default
                    // views' inputs connection until all the new sources have
        ...
        if (evUseFastAddSource.getValue() && isFastAddSourceEnabled())
        {
            // Fast add source mechanism which postpones the default views'
            // inputs connection until all the new sources have been added to
            // prevent O(n^2)
