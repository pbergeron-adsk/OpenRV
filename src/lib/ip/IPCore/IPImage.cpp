...
namespace IPCore
{
    ...
    FrameBuffer* IPImage::newBlankFrameBuffer(size_t w, size_t h)
    {
    ...
    FrameBuffer* IPImage::newBlackFrameBuffer(size_t w, size_t h)
    {
    ...
    FrameBuffer* IPImage::newNoImageFrameBuffer(size_t w, size_t h)
    {
    ...
    FrameBuffer* IPImage::newErrorFrameBuffer(size_t w, size_t h)
    {
        FrameBuffer* fb = newBlankFrameBuffer(w, h);
        fb->idstream() << "(ERROR)";
        return fb;
                                                         ...
                                                         const string& message)
    {
        ...
        if (node)
            fb->newAttribute<string>("Node", node->uiName());
        if (message != "")
    ...
    IPImageID* IPImage::newBlankImageID(size_t w, size_t h)
    {
        FrameBuffer* fb = newBlankFrameBuffer(w, h);
    ...
    IPImageID* IPImage::newBlackImageID(size_t w, size_t h)
    {
    ...
    IPImageID* IPImage::newNoImageID()
    {
    ...
    IPImageID* IPImage::newErrorImageID()
    {
    ...
    IPImage::getBlendModeFromString(const char* blendModeString)
    {
        ...
        if (!strcmp(blendModeString, "over"))
            blendMode = IPImage::Over;
        else if (!strcmp(blendModeString, "add"))
    ...
    bool IPImage::isDataBuffer() const { return destination == DataBuffer; }

    size_t IPImage::allocSize() const
    {
        size_t size = 0;
    ...
    size_t IPImage::totalImageSize() const
    {
        size_t size = 0;
    ...
    void IPImage::appendChildren(const std::vector<IPImage*>& images)
    {
        for (size_t i = 0; i < images.size(); i++)
            appendChild(images[i]);
    }

    size_t IPImage::numImages() const
    {
        size_t n = 0;
    ...
    size_t IPImage::numChildren() const
    {
        if (children)
            return children->numImages();
        else
            return 0;
    }

    float IPImage::displayPixelAspect() const
    {
        if (fb)
            return pixelAspect ? pixelAspect : fb->pixelAspectRatio();
        else
            return pixelAspect ? pixelAspect : 1.0;
    }
    ...
    void IPImage::recordResourceUsage()
    {
        if (mergeExpr)
        {
    ...
    IPImage::Matrix IPImage::fitToAspectMatrix(float aspect) const
    {
        Matrix M;
    ...
    void IPImage::fitToAspect(float aspect)
    {
        //
        //  Note, we don't want to use *= here, since that implies applying the
        //  "fit" matrix first, and we want to apply the fit matrix to the
        ...
        const InternalGLMatricesContext& baseContext)
    {
        ...
        if (next)
            next->computeMatricesRecursive(baseContext);
        ...
        if (children)
        {
            ...
            if (isIntermediateRender())
            {
                ...
                //
                //  leave global projection matrix alone, so it reflects the
                //  final projection to the screen (root), not an intermediate
                //  projection to a buffer.
                //
                                          ...
                                          size_t& coordNum) const
    {
        ...
        else if (renderType == BlendRenderType || renderType == GroupType)
        {
            //
    ...
    const string& IPImage::graphID() const { return m_graphID; }

    void IPImage::renderIDHashRecursive(ostream& o) { o << renderID(); }

    IPImage::HashValue IPImage::renderIDHash() const
    {
    ...
    IPImage::HashValue IPImage::fbHash() const
    {
        if (renderIDNeedsCompute())
            computeRenderIDs();
        return m_fbHash;
    ...
    const string& IPImage::renderID() const
    {
        if (renderIDNeedsCompute())
            computeRenderIDs();
        return m_renderID;
    ...
    void IPImage::computeRenderIDs() const
    {
        //
        ...
        if (pixelAspect != 0.0)
            o << "p!" << pixelAspect;
        if (!stencilBox.isEmpty())
            hashBox(o, stencilBox);
        o << "bm" << blendMode;
        ...
        //
        //  XXX Hash commands into renderID, but set aside ID prior to last
        //  paint command for "partial paint" ID.  BUT, need parentMatrix in
        //  partial paint ID regardless of number of paint commands.
        //
    ...
    void IPImage::assembleAuxFrameBuffers()
    {
        for (IPImage* child = children; child; child = child->next)
        {
    ...
    void IPImage::computeRenderIDRecursive()
    {
        for (IPImage* child = children; child; child = child->next)
        {
    ...
    static void printTreeRecursive(ostream& o, const IPImage* image, int level)
    {
        if (!image)
            return;
        ...
        {
        case IPImage::IntermediateBuffer:
            o << "[*|";
            break;
        case IPImage::TemporaryBuffer:
            o << "[+|";
            break;
        case IPImage::OutputTexture:
            o << "[T|";
            break;
        case IPImage::DataBuffer:
            o << "[D|";
            break;
        case IPImage::LeftBuffer:
            o << "[L|";
            break;
        case IPImage::RightBuffer:
            o << "[R|";
            break;
        case IPImage::MainBuffer:
            o << "[M|";
            break;
        default:
        case IPImage::CurrentFrameBuffer:
            o << "[ |";
            break;
        }
        ...
        {
        default:
        case IPImage::GroupType:
            o << "G|";
            break;
        case IPImage::BlendRenderType:
            o << "B|";
            break;
        case IPImage::MergeRenderType:
            o << "M|";
            break;
        case IPImage::NoRenderType:
            o << "-|";
            break;
        case IPImage::RecordOnlyType:
            o << "R|";
            break;
        case IPImage::ExternalRenderType:
            o << "E|";
            break;
        }
        ...
        {
        case IPImage::NoInternalDataType:
            o << "?ty";
            break;
        ...
        if (image->pixelAspect != 1.0)
            o << "|Pa";
        if (image->transformMatrix != IPImage::Matrix())
            o << "|Tf";
    ...
    void IPImageID::append(IPImageID* img)
    {
        IPImageID* i;
        for (i = this; i->next; i = i->next)
            ;
    ...
    void IPImageID::appendChild(IPImageID* img)
    {
        if (children)
            children->append(img);
        else
    ...
    void IPImageID::appendChildren(const std::vector<IPImageID*>& images)
    {
        for (size_t i = 0; i < images.size(); i++)
            appendChild(images[i]);
    }
