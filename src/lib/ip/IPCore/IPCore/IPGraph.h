...
namespace IPCore
{
    ...
    public:
        ...
        //  {
        //      string filename = ...;
        //      size_t workID = graph().addWorkItem(bind(&MyIPNode::read, this,
        //      filename), "openfile")
        //  }
        //
        ...
        {
            NodeValidation(IPNode* n)
                : node(n)
            {
                node->graph()->beginNodeValidation(node);
            }
        ...
        //
        //  This is the viewGroup not the viewNode (which is the input to the
        //  viewgroup).
        //
        ...
        //  1) #Type.*      -> all props in Node of Type along all rendering
        //  paths 2) @Type.*      -> first prop in Node of Type, along all
        //  rendering paths 3) :Type.*      -> prop in definition of Type 4)
        //  nodeName/... -> prefixed by nodeName/ root lookup at nodeName
        ...
        struct EvalProfilingRecord
        {
            EvalProfilingRecord()
                ...
                , ioStart(0)
                , ioEnd(0)
                , restartThreadsAStart(0)
                , restartThreadsAEnd(0)
                , restartThreadsBStart(0)
                , restartThreadsBEnd(0)
                , cacheTestLockStart(0)
                , cacheTestLockEnd(0)
                , setDisplayFrameStart(0)
                , setDisplayFrameEnd(0)
                , frameCachedTestStart(0)
                , frameCachedTestEnd(0)
                , awakenThreadsStart(0)
                , awakenThreadsEnd(0)
            {
            }
        ...
        // actual media loading is deferred until after the
        // RvSession::continueLoading() has completed. Now the problem here, is
        // that the caching might be restarted too soon and the media loading
        // and the cache would be fighting which would lead to the main thread
        // being unnecessarily blocked for long periods of time which would
        // defeat the purpose of the cache which is to optimize performances.
        // This is especially an issue when playing back media from URL or high
        // latency media storage.
        // By keeping track of when we still have media loading asynchronously,
        // we will be able to restart caching at the right time. Note that the
        // following three methods are thread safe with respect to the set
        // container used to hold the list of async media loading in progress.

        // Returns true if there is still media being loaded asynchronously
        // otherwise returns false.
        bool isMediaLoading() const;

        // Indicate that the specified media has been scheduled for async
        // loading
        ...
        , m_audioFlush(false)
    {
        for (size_t i = 0, s = props.size(); i < s; i++)
        {
                    ...
                    dynamic_cast<const PropertyInfo*>(props[i]->info()))
            {
                ...
                if (pinfo->requiresProgramFlush())
                {
                    m_programFlush = true;
                }
                if (pinfo->requiresAudioFlush())
                {
                    m_audioFlush = true;
                }
            }
